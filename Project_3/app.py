...
# Create our session (link) from Python to the DB
  ...
  # results = session.query(*sel).order_by(Cities.population.desc()).limit(100).all()
    ...
    for result in results2:
        ...
        ml_metadata_dict["biz_gagriculture_forest_fish_hunt_ratiorowth_Y"] = result[8]
        ...
        ml_metadata_dict["professional_scientific_tech_services_ratio"] = result[19]
        ml_metadata_dict["mgmt_companies_enterprises_ratio"] = result[20]
        ml_metadata_dict["admin_support_waste_management_services_ratio"] = result[21]
        ml_metadata_dict["educational_services_ratio"] = result[22]
        ml_metadata_dict["health_social_assistance_ratio"] = result[23]
        ml_metadata_dict["arts_entertainment_and_recreation_ratio"] = result[24]
